...
// For static exports (mobile builds), NEXTAUTH_URL warnings are suppressed during build
// The API routes will be hosted on the server specified in capacitor.config.ts (https://zuni.social)
const isStaticExport = process.env.NEXT_PHASE === 'phase-production-build';
const isMobileBuild = process.env.CAPACITOR === 'true';
const nextAuthUrl = process.env.NEXTAUTH_URL;

// Only show warnings in development or non-static builds
if (!isStaticExport || (!isMobileBuild && nextAuthUrl)) {
  if (!nextAuthUrl && !isStaticExport) {
